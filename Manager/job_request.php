...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['request_job'])) {
    ...
    // Insert job request into the database
    $request_query = "INSERT INTO job_requests (job_id, user_id) VALUES (:job_id, :user_id)";
    $request_stmt = $connect->prepare($request_query);
    $request_stmt->bindParam(':job_id', $job_id);
    $request_stmt->bindParam(':user_id', $user_id);

    if ($request_stmt->execute()) {
        $success_message = "Job request submitted successfully!";
    } else {
        $error_message = "Failed to submit job request.";
    }
}
?>
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h5 class="card-title">Job /Field Postings</h5>
                        </div>
                        <div class="card-body">
                            ...
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <?php if ($job['image_path']): ?>
                                                        <img src="uploads/<?php echo htmlspecialchars($job['image_path']); ?>" alt="Job Image" width="100">
                                                    <?php else: ?>
                                                        No Image
                                                    <?php endif; ?>
                                                </td>
                                                <td>
                                                    <form action="view_jobs.php" method="POST">
                                                        ...
                                                        <button type="submit" name="request_job" class="btn btn-primary">Request Job</button>
                                                    </form>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php else: ?>
                                        <tr>
                                            <td colspan="9" class="text-center">No job postings found.</td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
